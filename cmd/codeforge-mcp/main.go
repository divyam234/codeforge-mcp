...
import (
	...
	"github.com/divyam234/codeforge-mcp/internal/config"
	...
	"github.com/divyam234/codeforge-mcp/internal/tools"
)
...
func run() int {
	...
	logger.Info("CodeForge MCP starting", "transport", "http", "http_address", cfg.HTTPAddress, "http_auth", cfg.APIKey != "", "workspace_root", cfg.WorkspaceRoot, "state_dir", cfg.StateDir, "active_project", projects.ActiveID(), "version", version)

	if err := server.RunHTTP(ctx, cfg.HTTPAddress); err != nil && !errors.Is(err, context.Canceled) && ctx.Err() == nil {
		logger.Error("MCP HTTP server failed", "error", err, "address", cfg.HTTPAddress)
		return 1
	}
	return 0
}
